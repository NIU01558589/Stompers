...
if (!empty($_SESSION['total_productes']['info_producte'])) {
    foreach ($_SESSION['total_productes']['info_producte'] as $producto) {
        $aux_un_preu = $producto['preu']*$producto['quantitat'];
        echo "<div id='carrito-container'>";
        echo "<a href='modifications.php?action=quitar&producto_id={$producto['id']}'><ion-icon name='close-outline'></ion-icon></a>";
        echo "<img src='{$producto['foto']}' alt='Imagen de {$producto['nom']}'/>";
        echo "<p>Nombre: {$producto['nom']}</p>";
        echo "<p>Precio: {$aux_un_preu}€</p>";
        echo "<a href='modifications.php?action=sumar&producto_id={$producto['id']}'><ion-icon name='add-outline'></ion-icon></a>";
        echo "<p>Cantidad: {$producto['quantitat']}</p>";
        echo "<a href='modifications.php?action=restar&producto_id={$producto['id']}'><ion-icon name='remove-outline'></ion-icon></a>";
        echo "</div>";
        echo "<hr>";
    }
    ...
    echo "<div id='agrupacio_botons'>";
    echo "<form action='../controller/finalizar_compra.php' method='post'>";
    echo "<button id='finalitzar_compra_estil' type='submit' value='Finalizar Compra'>Finalitzar Compra</button>";
    echo "</form>";

    echo "<form action='modifications.php?action=vaciar_carrito' method='post'>";
    echo "<button type='submit' value='Vaciar Cesta'>Vaciar Cesta</button>";
    echo "</form>";
    echo "</div>";
} else {
    echo "<p id='vacio_carro'>El carrito está vacío.</p>";
}
